...
export function Navbar() {
  ...
  const navTarget = (id: string) => (isHome ? undefined : `/#${id}`);
          ...
          {links.map((l) => (
            <li key={l.id}>
              {isHome ? (
                ...
                <Link
                  to={navTarget(l.id)!}
                  className="group relative px-4 py-2 rounded-full transition-colors hover:text-primary"
                >
                  <span className="relative z-10">{l.label}</span>
                </Link>
              )}
              ...
              <li key={l.id}>
                {isHome ? (
                  ...
                  <Link
                    ...
                    className="block w-full text-left px-4 py-3 rounded-2xl hover:bg-vanilla/60 font-display text-2xl"
                  >
                    {l.label}
                  </Link>
                )}
